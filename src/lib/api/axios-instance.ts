...
const decodeToken = (value: string): string => {
	try {
		// Tokens are stored via LocalStorageRepositoryImpl using base64 encoding
		// Decode if possible, otherwise return raw value
...
axiosInstance.interceptors.request.use(
	(config) => {
		if (typeof window !== 'undefined') {
			...
			const token = stored ? decodeToken(stored) : null;
			if (token) {
